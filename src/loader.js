...
/** @const {number} */ Loader.PX_PER_MM = 5;
...
/** @const {Object<number>}*/ Loader.A3SIZE = Loader.PAGESIZES[12];
...
/** @const Array<string> */ Loader.PAGESIZENAMES = [
    ...
    "L Portrait",
    "XL Portrait",
    "Fit to page width" // has an undefined value in pagesizes table
];
Loader.prototype.printSizeSelector = function (attrs, parent) {
    var onchange = (evt) => {
        // Will have a null value for "fit to page size"
        let size = Loader.PAGESIZES[evt.currentTarget.value];
        this.pageSize = size ? size : null;
    }
    Draw.dropDownSegment(
        "Zoom Level:", onchange, Loader.PAGESIZENAMES, attrs, parent);
};
Loader.prototype.print = function () {
    let trakMap = new TrakMap(this.trakMap.save());
    trakMap.draw();

    if (this.pageSize === null) { // fit to width
        ...
        for (var left = trakMap.getLeft(); left < right; left += width * 0.9) {
            ...
            for (var top = trakMap.getTop(); top < bottom; top += height * 0.9) {
                ...
                elem.children[0].setAttribute("viewBox", "" +
                    left + " " + top + " " +
                    width + " " + height + " ");
            }
        }
